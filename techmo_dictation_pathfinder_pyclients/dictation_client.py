...
if __name__ == '__main__':
    ...
    print("Read file \"{args.wave}\" with sampling rate {sampling_rate} and {len(audio)} bytes of data.")

    # Setup connection
    channel = grpc.insecure_channel(args.address)
    dictation = speech_grpc.SpeechStub(channel)
    print("Setup connection with dictation service at address {args.address}")
    ...
    for recognition in recognitions_iterator:
        ...
        for result in recognition.results:
            ...
            try:
                ...
                time_str = "[{begin}ms - {end}ms]"
            except:
                time_str = ""

            # determine confidence
            confidence = "({best_alternative.confidence:.2f})" if best_alternative.confidence != 0. else ""
